...
class GitModificationParserTest {
    ...
    private GitModificationParser parser = new GitModificationParser();
    ...
    @Test
    void shouldCreateModificationForEachCommit() {
        simulateOneComment();
        assertThat(parser.getModifications().get(0).getRevision()).isEqualTo("4e55d27dc7aad26dadb02a33db0518cb5ec54888");
    }

    @Test
    void shouldHaveCommitterAsAuthor() {
        simulateOneComment();
        assertThat(parser.getModifications().get(0).getUserDisplayName()).isEqualTo("Cruise Developer <cruise@cruise-sf3.(none)>");
    }

    @Test
    void shouldHaveCommitDate() {
        simulateOneComment();
        assertThat(parser.getModifications().get(0).getModifiedTime()).isEqualTo(DateUtils.parseISO8601("2009-08-11 13:08:51 -0700"));
    }
    ...
    void shouldHaveComment() {
        ...
        parser.processLine("");
        assertThat(parser.getModifications().get(0).getComment()).isEqualTo("My Comment");
    }

    @Test
    void shouldSupportMultipleLineComments() {
        simulateOneComment();
        parser.processLine("");
        parser.processLine("    My Comment");
    ...
    @Test
    void shouldSupportMultipleLineCommentsWithEmptyLines() {
        simulateOneComment();
        parser.processLine("");
        parser.processLine("    My Comment");
        parser.processLine("    ");
    ...
    @Test
    void shouldSupportMultipleModifications() {
        simulateOneComment();
        parser.processLine("");
        ...
        parser.processLine("");
        assertThat(parser.getModifications().get(0).getComment()).isEqualTo("My Comment 1");
        assertThat(parser.getModifications().get(1).getComment()).isEqualTo("My Comment 2");
    }
}
