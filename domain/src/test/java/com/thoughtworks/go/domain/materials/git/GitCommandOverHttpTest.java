...
class GitCommandOverHttpTest {
    ...
    void shouldConnectToRemoteHttpRepository() {
        ...
        Assertions.assertThatCode(() -> {
            gitCommand.checkConnection(new UrlArgument(publicHttpUrl()), "master", Collections.emptyMap());
    ...
    void shouldFailOnBadHttpRepositoryUrl() {
        ...
        Assertions.assertThatCode(() -> {
            gitCommand.checkConnection(new UrlArgument(badHttpUrl()), "master", Collections.emptyMap());
        })
    ...
    void shouldConnectToHttpUrlWithAuthorization() {
        ...
        Assertions.assertThatCode(() -> {
            String urlWithCredentials = UrlDenormalizerXSLTMigration121.urlWithCredentials(privateHttpUrl(), BasicAuthenticationFilter.LOGIN_USER, BasicAuthenticationFilter.LOGIN_PASSWORD);
            gitCommand.checkConnection(new UrlArgument(urlWithCredentials), "master", Collections.emptyMap());
        }).doesNotThrowAnyException();
    }
    ...
    void shouldFailWithBadAuthenticationOnHttp() {
        ...
        Assertions.assertThatCode(() -> {
            String urlWithCredentials = UrlDenormalizerXSLTMigration121.urlWithCredentials(privateHttpUrl(), "bad", "hacker");
            gitCommand.checkConnection(new UrlArgument(urlWithCredentials), "master", Collections.emptyMap());
        })
                .hasMessageContaining("fatal")
    ...
    void shouldFailWithBadAuthenticationOnHttpWhenCredentialsNotProvided() {
        ...
        Assertions.assertThatCode(() -> {
            gitCommand.checkConnection(new UrlArgument(privateHttpUrl()), "master", Collections.emptyMap());
        })
                .hasMessageContaining("fatal")
                .hasMessageContaining("Authentication failed for 'http://localhost:" + getServerPort() + "/git/private/my-project/'");
    }
